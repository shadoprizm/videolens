...
describe("managed recipe lookup", () => {
  ...
  it("preserves the production model reasoning settings through managed chat", async () => {
    vi.stubEnv("OPENAI_API_KEY", "managed-test-key");
    const fetch = vi.spyOn(globalThis, "fetch").mockImplementation(async () => new Response(JSON.stringify({ choices: [{ message: { content: "{}" } }] })));
    try {
      for (const reasoning_effort of ["none", "medium"]) {
        const response = await handler(request({ kind: "chat", reportId: "report", model: "gpt-5.6-terra", messages: [{ role: "user", content: "Analyze" }], jsonObject: true, reasoning_effort }));
        expect(response.status).toBe(200);
        const sent = JSON.parse(fetch.mock.calls.at(-1)![1]?.body as string);
        expect(sent).toMatchObject({ model: "gpt-5.6-terra", reasoning_effort, response_format: { type: "json_object" } });
        expect(sent).not.toHaveProperty("temperature");
      }
    } finally { vi.unstubAllEnvs(); }
  });
  it("rejects unsupported reasoning settings before consuming a request", async () => {
    const response = await handler(request({ kind: "chat", reportId: "report", model: "gpt-5.6-terra", messages: [], reasoning_effort: "unsupported" }));
