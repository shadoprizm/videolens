// Evaluate real downloaded video evidence with shipping Procedure code.
// node --env-file=../site/.env.evaluation.local scripts/evaluate-procedure.mjs VIDEO OUTPUT_DIR [CAPTIONS_JSON3] [INFO_JSON]
...
if(!videoArg||!outArg)throw Error('Usage: evaluate-procedure.mjs VIDEO OUTPUT_DIR [CAPTIONS_JSON3] [INFO_JSON]');
...
const [p,v,t,a,r,config]=await Promise.all(['procedure','describeProcedureFrames','timeline','analyze','report','config'].map(load));
...
const times=p.procedureFrameTimestamps(duration,transcript),frames=[];
...
const observations=existsSync(obsPath)?JSON.parse(readFileSync(obsPath,'utf8')):await v.describeProcedureFrames(access,frames,(done,total)=>{if(done%30===0||done===total)console.log({stage:'vision',done,total});});
...
const analysis=await a.analyzeTimeline(access,timeline,source,'tutorial','Make a reusable procedure. Preserve exact demonstrated values and explicitly identify missing information.','en');
...
const summary={models:config.MODELS,evaluationLimitations:source.limitations,source:source.url,title:source.title,duration,frames:frames.length,observations:observations.length,hasCaptions:!!transcript,steps:analysis.procedure.steps.length,gaps:analysis.procedure.gaps,...stats};writeFileSync(resolve(out,'run.json'),JSON.stringify(summary,null,2));console.log(summary);
